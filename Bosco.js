...
module.exports = bosco = async (bosco, mek) => {
	try {
		...
		const catl = (teks) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 400, "message": teks, "footerText": "*Sojin Ser*", "thumbnail": dfrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             bosco.relayWAMessage(res)
        }
       const catlo = (teks) => {
             res = bosco.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 70000, "message": teks, "footerText": "Made With Sojin", thumbnail: fs.readFileSync('./ds.jpg'), "surface": 'CATALOG' }}, {quoted:mek})
             bosco.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = bosco.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/HebsCx7CBxMJBLqyeHemcO', "groupName": `SojinSER`, "footerText": "*Sojin Ser*", "jpegThumbnail": dfrply, "caption": teks}}, {quoted:fvideo})
...
// AUTO
			...
			for (let anjh of imagi){
				if (budy === anjh){
					...
					bosco.sendMessage(from, result, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { orderMessage: { itemCount: 500, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `${fake}`, orderTitle: `MADE BY SojinSER`, sellerJid: '0@s.whatsapp.net'}}}})
					}
			}
      ...
      const sendFileFromUrl = async(link, type, options) => {
          ...
          if (!packname) packname = '!Sojin'; if (!author) author = 'Ser';author = author.replace(/[^a-zA-Z0-9]/g, '');	
...
// Anti link
       ...
       if (isGroup && isAntiLink && !isOwner && !isGroupAdmins && isBotGroupAdmins){
        ...
        if (isCmd && isGroup)
            ...
            case 'owner':
            ini_ownerNumber = [`${setting.owner}@s.whatsapp.net`,`917994873071@s.whatsapp.net`,`19014016638@s.whatsapp.net`]
            let ini_list = []
		    for (let i of ini_ownerNumber) {
			const vname_ = bosco.contacts[i] != undefined ? bosco.contacts[i].vname || bosco.contacts[i].notify : undefined
		     ini_list.push({
			 "displayName": 'Owner SojinSER',
			 "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname_ ? `${vname_}` : `${bosco.user.name}`}\nORG: Sojin Ser;\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Its me Sojin\nEND:VCARD`
			  })
			  }
...
//------------------< Bot Owner >-------------------
       ...
       case 'group':
        rows = [
          ...
          section = [{title: "Sᴜʙsᴄɪʙᴇ Yᴛ ᴀᴊᴜx Fᴏʀ ᴍᴏʀᴇ Uᴘᴅᴀᴛᴇs", rows: rows}]
	...
	case 'tagall':
      ...
      case 'hidetag':
       ...
       case 'Sojin':
              if (!q) return
              qq = q.toUpperCase()
              awikwok = `${qq} ${qq} ${qq} ❤️ ❤️ ❤️ WANGY WANGY WANGY WANGY HU HA HU HA HU HA, Sojin Poli Ahn ${qq} Sojin Killadi Ahnu ${qq} AAAAAAAAH ~ Rambutnya.... aaah rambutnya juga pengen aku elus-elus ~~ AAAAAH ${qq} keluar pertama kali di anime juga manis ❤️ ❤️ ❤️ banget AAAAAAAAH ${qq} AAAAA LUCCUUUUUUUUUUUUUUU............ ${qq} AAAAAAAAAAAAAAAAAAAAGH ❤️ ❤️ ❤️apa ? ${qq} itu gak nyata ? Cuma HALU katamu ? nggak, ngak ngak ngak ngak NGAAAAAAAAK GUA GAK PERCAYA ITU DIA NYATA NGAAAAAAAAAAAAAAAAAK PEDULI BANGSAAAAAT !! GUA GAK PEDULI SAMA KENYATAAN POKOKNYA GAK PEDULI. ❤️ ❤️ ❤️ ${qq} gw ... ${qq} di laptop ngeliatin gw, ${qq} .. kamu percaya sama aku ? aaaaaaaaaaah syukur ${q} aku gak mau merelakan ${qq} aaaaaah ❤️ ❤️ ❤️ YEAAAAAAAAAAAH GUA MASIH PUNYA ${qq} SENDIRI PUN NGGAK SAMA AAAAAAAAAAAAAAH`
              reply(awikwok)
              break
        case 'jadibot':
              if (!isOwner) return
...
//------------------< Downloader/And Search Media >-------------------
      ...
      case 'more':
       ...
       case 'pinterest':
         case 'pin':
              if (args.length < 1) return reply(`${prefix}Sojin Ser`)
...
`
 ...
 menumessage = { contentText: `${teks}`, footerText: `ғᴏʟʟᴏᴡ ᴍᴇ ɪɴ𝗌ᴛᴀ 𝗌ᴏ_ᴊɪɴ`, buttons: menubutton, headerType: 6, locationMessage: bosco2.message.locationMessage}
       ...
       case "anime":
        ...
        case 'waifu':
          ...
          case 'wallml':
              ...
              imageMsg = ( await bosco.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'Dont forget to Subscribe SojinX', imageMessage: imageMsg,
...
case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} Sojinser`)
       ...
       case 'sgif':
              ...
              if (err) return reply(`${err}`)
              exec(`webpmux -set exif ${addMetadata('Sojin')} ${ron} -o ${ron}`, async (error) => {
              if (error) return reply(`${error}`)
...
//------------------< Storage >-------------------
      ...
      case 'addbgm':
					...
					bosco.sendMessage(from, `*Bgm Added*`, MessageType.text, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: 'status@broadcast' } : {})}, message: { orderMessage: { itemCount: 333, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `${nm}`, orderTitle: 'Sojin', sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true}})
					break
	 case 'delvn':
					try {
					 nmm = body.slice(7)
				...
				case 'listbgm':
					...
					teks += `\n*Tᴏᴛᴀʟ : ${vien.length}*\n\n_▢_`
					bosco.sendMessage(from, teks.trim(), extendedText, { caption: 'teks', "contextInfo": { text: 'teks', sendEphemeral: true, "externalAdReply": { "title": `${' '}𝙎𝙊𝙅𝙄𝙉𝙎𝙀𝙍${''}${''}`, "body": ``, "previewType": 'PHOTO', "thumbnailUrl": `${'https://i.ibb.co/kQ98t1k/Sojin.jpg'}`, "thumbnail": '', "sourceUrl": `${'https://wa.me/c/19014016638'}`}},quoted: ftext})
					break
				case 'addimage':
...
//------------------< Group >-------------------
       ...
       case 'block':
        ...
        case 'getpp':
               ...
               bosco.sendMessage(from, buffer, image, {caption: "©SojinSER", quoted: mek })
...
//------------------< Maker Menu >-------------------
                ...
                case 'horrorblood':
                case 'thunder':
                    if (args.length == 0) return reply(`Example: ${prefix + command} Sojin Ser`)
                    ini_txt = args.join(" ")
...
if (Number(oi2) >= 50) return reply('*Most!*')
	if (!Number(oi2)) return reply('*The number must be a number!*')
     ...
     case 'mute':
                ...
                break
     case 'tts':
					if (args.length < 1) return bosco.sendMessage(from, `Where's the language cod ? contoh : ${prefix}tts ml Sojinser uyir`, text, { quoted: mek })
				   const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return bosco.sendMessage(from, `Where's the Texk bro? contoh : ${prefix}tts ml Sojinser uyir`, text, { quoted: mek })
